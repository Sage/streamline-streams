	...
	proto.fork = function(consumers, options) {
		...
		function consume(item) {
			if (++item.readCount === consumers.length) {
				...
				bufferBase++;
				// rearm read
				if (buffer.length === 0 && q.length > 0 && next && !waiting) {
					waiting = true;
		...
		function flushQueue(item) {
			...
			while (queued = q.shift()) {
				...
				consume(item);
				streamsPositions[queued.i]++;
				queued.cb(item.err, item.val);
			}
		}
		...
		var sources = consumers.map(function(consumer, i) {
			...
			function read(cb) {
				...
				if (offset >= 0 && offset < buffer.length) {
					...
					consume(item);
					streamsPositions[i]++;
					cb(item.err, item.val);
				} else if (buffer.length < bufSize && !waiting) {
					if (next) {
						trace && trace("FORK: " + i + ": fetching");
						waiting = true;
						next(function(err, val) {
							waiting = false;
							trace && trace("FORK: " + i + ": next returns err=" + err + ", val=" + val);
							var item = {
								readCount: done,
								err: err,
								val: val,

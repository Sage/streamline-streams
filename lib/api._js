...
exports.decorate = function(proto) {
	...
	proto.fork = function(consumers) {
		...
		var sources = consumers.map(function(consumer) {
			...
			function read(cb) {
				trace && console.log("inside read: pos=" + pos + ", position=" + position);
				var i = pos - position;
				var result;
				if (i < 0) throw new Error("invalid state 1 in fork: i=" + i);
				if (i >= 0 && i < results.length) {
					trace && console.log("found in results");
					result = results[i];
					...
					if (next) {
						...
						trace && console.log("at end");
						cb(null);
					}
				} else {
					trace && console.log("queuing");
					q.push(cb);
				}
			}
			...
			return new Decorated(function(_) {
				...
				trace && console.log("end of chain: " + val);
				if (val === undefined) {
					done++;
				}
				return val;
	...
	proto.parallelize = function(count, consumer) {
		var self = this;
		var streams = [];
		for (var i = 0; i < count; i++) streams.push(consumer(new Decorated(self.read.bind(self))));
		return new StreamGroup(streams).dequeue();
	};

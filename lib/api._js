...
exports.decorate = function(proto) {
	...
	proto.fork = function(consumers, options) {
		...
		var sources = consumers.map(function(consumer, i) {
			...
			function read(cb) {
				trace && trace("FORK: inside read: pos=" + positions[i] + ", position=" + position);
				...
				} else if (results.length < bufSize) {
					if (next) {
						trace && trace("FORK: fetching");
						next(function(err, val) {
							trace && trace("FORK: next returns err=" + err + ", val=" + val);
							var result = {
								count: done + 1,
								err: err,
								val: val,
							...
							while (queued = q.shift()) {
								...
								consume(result);
								positions[queued.i]++;
								queued.cb(result.err, result.val);
							}
